...
describe('create', () => {
  ...
  it('should throw forbidden error when there is no enrollment for given user', async () => {
    ...
    const promise = bookingService.create(inputBooking.userId, inputBooking.roomId);
  ...
  it('should throw not found error when room does not exist', async () => {
    jest.spyOn(hotelRepository, 'findRoomWithBookingCountById').mockResolvedValueOnce(null);
    jest.spyOn(enrollmentRepository, 'findWithAddressByUserId').mockResolvedValueOnce(enrollment);
    jest.spyOn(ticketsRepository, 'findTicketByEnrollmentId').mockResolvedValueOnce(ticket);
    jest.spyOn(bookingRepository, 'findByUserId').mockResolvedValueOnce(null);
  ...
  it('should throw forbidden error when room is at full capacity', async () => {
    ...
    jest.spyOn(ticketsRepository, 'findTicketByEnrollmentId').mockResolvedValueOnce(ticket);
    jest.spyOn(bookingRepository, 'findByUserId').mockResolvedValueOnce(null);
    jest.spyOn(hotelRepository, 'findRoomWithBookingCountById').mockResolvedValueOnce({
      capacity: 5,
      _count: {
        Booking: 5,
      },
...
describe('updateBooking', () => {
  ...
  it('should return booking id when successful', async () => {
    jest.spyOn(bookingRepository, 'findByUserId').mockResolvedValueOnce(booking);
    jest.spyOn(hotelRepository, 'findRoomWithBookingCountById').mockResolvedValueOnce(room);
    jest.spyOn(bookingRepository, 'updateByBookingIdAndRoomId').mockResolvedValueOnce(booking);
